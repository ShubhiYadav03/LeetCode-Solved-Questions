...
class Solution {
    ...
    Set<String> set = new HashSet(op);
    public int evalTree(Node root) {
        if(root == null) return 0;
        //if((root.left == null && root.right == null) || !set.contains(root.val)) return Integer.parseInt(root.val);
        
        int left = evalTree(root.left);
        int right = evalTree(root.right); 
        
        if(root.data.equals("*")) return left * right;
        else if(root.data.equals("+")) return left + right;
        else if(root.data.equals("-")) return left - right;
        else if(root.data.equals("/")) return left / right;
